...
const BookingCalendar = ({ refreshKey }) => {
  ...
  const calendarEvents = bookings
    ...
    .map(booking => {
      ...
      const borderColor = STATUS_BORDER_COLORS[booking.status] || bgColor;
      ...
      <div className="flex flex-wrap gap-4 mb-4 text-sm">
        ...
        {categoryOptions.filter(o => o.value !== 'all').map(({ value, label }) => (
          <span key={value} className="flex items-center gap-1.5">
            <span className="inline-block w-3 h-3 rounded-sm" style={{ backgroundColor: CATEGORY_COLORS[value] }} />
            {label}
          </span>
        ))}
        <span className="ml-4 font-semibold text-gray-600">Status (border):</span>
        {Object.entries(STATUS_BORDER_COLORS).map(([status, color]) => (
          <span key={status} className="flex items-center gap-1.5">
            <span className="inline-block w-3 h-3 rounded-sm border-2" style={{ borderColor: color, backgroundColor: 'white' }} />
            {status.charAt(0).toUpperCase() + status.slice(1)}
          </span>
        ))}
      </div>

      <div ref={calendarWrapperRef} className="border rounded-lg overflow-hidden">
        <FullCalendar
          ...
          eventDidMount={(info) => {
            const { category, booking } = info.event.extendedProps;
            const bgColor     = CATEGORY_COLORS[category] || '#6b7280';
            const borderColor = STATUS_BORDER_COLORS[booking?.status] || bgColor;
