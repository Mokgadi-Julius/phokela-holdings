...
const BookingCalendar = ({ refreshKey }) => {
  ...
  // Format bookings for FullCalendar — memoised so FullCalendar only re-renders
  // when the underlying data or one of the three filters actually changes.
  const calendarEvents = useMemo(() => bookings
    ...
    .filter(Boolean),
  [bookings, filterStatus, filterCategory, filterServiceId]);
  ...
  const handleEventDrop = (info) => {
    ...
    if (isAccommodation) {
      const origCheckInStr  = toDateStr(booking.bookingDetails?.checkIn);
      const origCheckOutStr = toDateStr(booking.bookingDetails?.checkOut);
      if (origCheckInStr && origCheckOutStr) {
        // Compute how many whole days the event was shifted, then apply the
        // same delta to the original check-out date. This correctly handles
        // same-day bookings (checkIn == checkOut as date strings) as well as
        // multi-night stays without relying on FullCalendar's exclusive-end date.
        const [oiy, oim, oid] = origCheckInStr.split('-').map(Number);
        const [niy, nim, nid] = newStart.split('-').map(Number);
        const deltaDays = Math.round(
          (Date.UTC(niy, nim - 1, nid) - Date.UTC(oiy, oim - 1, oid)) / 86400000
        );
        const [ocy, ocm, ocd] = origCheckOutStr.split('-').map(Number);
        const shifted = new Date(Date.UTC(ocy, ocm - 1, ocd + deltaDays));
        newCheckOut = `${shifted.getUTCFullYear()}-${String(shifted.getUTCMonth() + 1).padStart(2, '0')}-${String(shifted.getUTCDate()).padStart(2, '0')}`;
      } else {
        newCheckOut = info.event.end ? fcEndToCheckOut(info.event.end) : newStart;
      }
    }
    ...
    setPendingDrop({
      ...
      newCheckOut:  isAccommodation ? newCheckOut : null,
  ...
  const confirmDrop = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setDropError(response.message || 'Failed to reschedule booking.');
      }
    } catch (err) {
      setDropError(err?.data?.message || err?.message || 'An error occurred while rescheduling.');
    } finally {
      setDropSaving(false);
    }
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      <style>{`
        ...
        .booking-calendar-wrap .fc-popover-body {
          max-height: 260px;
          overflow-y: auto;
      ...
      <div ref={calendarWrapperRef} className="booking-calendar-wrap border rounded-lg">
        <FullCalendar
