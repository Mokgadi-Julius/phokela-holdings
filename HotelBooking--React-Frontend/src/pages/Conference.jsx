...
const Conference = () => {
  ...
  const policyIntro      = cms?.policy_intro            || 'All conference and meeting bookings are handled through a personalised inquiry process — not an online checkout. Contact us via WhatsApp or email to discuss your requirements, and we will prepare a custom contract before any deposit is taken.';
  ...
  const venue1Desc       = cms?.venue1_description      || 'Ideal for small meetings, interviews, and training sessions. The final amount is confirmed in your contract based on extras selected (catering, equipment, etc.).';
  const venue2Name       = cms?.venue2_name             || 'Conference Venue';
  ...
  const venue2Desc       = cms?.venue2_description      || 'Suitable for larger conferences, workshops, and corporate functions — indoors or outdoors. The final contract amount varies depending on your chosen package and extras.';
  const packagesSummary  = cms?.policy_packages_summary || 'Available packages include: venue only · venue + catering · venue + equipment + catering (full service) · conference/meeting without food · event without food and décor';
