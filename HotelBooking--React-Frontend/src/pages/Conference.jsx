  ...
  const fetchConferenceServices = async () => {
    try {
      ...
      try {
        ...
        console.warn('Conference API failed, using local fallback');
      }

      // Merge with local storage
      const localServices = JSON.parse(localStorage.getItem('local_services') || '[]')
        .filter(s => s.category === 'conference');
      ...
      if (combined.length > 0) {
        setServices(combined);
         ...
         // Fallback
         const conferenceServices = roomData.filter(item => item.name.includes('Conference'));
         setServices(conferenceServices);
      }
    } catch (error) {
