...
const Dashboard = () => {
  ...
  // Returns { label: '+8.5%', positive: true } or null when there's no meaningful comparison
  const calcChange = (current, last) => {
    ...
    return { label: `${pct >= 0 ? '+' : ''}${rounded}%`, positive: pct >= 0 };
  };
  ...
  const statCards = [
    ...
    },
    {
      ...
      icon: '💰',
      color: 'bg-purple-500',
      change: calcChange(stats.thisMonthRevenue, stats.lastMonthRevenue),
    },
    {
      title: 'New Contacts',
    ...
    },
    {
      ...
      icon: '💸',
      color: 'bg-red-500',
      change: calcChange(stats.thisMonthExpenses, stats.lastMonthExpenses),
      link: '/admin/expenses',
    },
    {
      title: 'Net Profit',
      ...
      change: calcChange(stats.thisMonthNetProfit, stats.lastMonthNetProfit),
    },
  ];
          ...
          <div key={index} className="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition">
            <div className="flex items-center justify-between">
              <div>
                ...
                {stat.change && (
                  <p className={`mt-2 text-sm flex items-center ${stat.change.positive ? 'text-green-600' : 'text-red-500'}`}>
                    ...
                    {stat.change.label} from last month
                  </p>
                )}
                {stat.link && (
