...
router.get('/', auth, async (req, res) => {
  try {
    ...
    if (category) {
      // Filter on the serviceSnapshot JSON column so accommodation bookings
      // (which have serviceId = null but roomId set) are included correctly.
      // Whitelist category values to prevent SQL injection.
      const allowedCategories = ['accommodation', 'conference', 'events', 'catering'];
      if (allowedCategories.includes(category)) {
        where[Op.and] = sequelize.literal(
          `JSON_UNQUOTE(JSON_EXTRACT(\`Booking\`.\`serviceSnapshot\`, '$.category')) = '${category}'`
        );
      }
    }
    ...
    // Whitelist sort fields — JSON path fields need sequelize.literal
    const sortFieldMap = {
      'createdAt':               sequelize.literal('`Booking`.`createdAt`'),
      'bookingDetails.checkIn':  sequelize.literal("JSON_UNQUOTE(JSON_EXTRACT(`Booking`.`bookingDetails`, '$.checkIn'))"),
    };
    const sortExpr  = sortFieldMap[sortBy] || sequelize.literal('`Booking`.`createdAt`');
...
router.patch('/:id/dates', auth, async (req, res) => {
  try {
    ...
    if (isAccommodation) {
      ...
      // For same-day bookings (checkIn === checkOut as date strings) the difference
      // is 0 ms, so we fall back to the original nights count to preserve pricing.
      const diffNights = Math.ceil((checkOutDate - checkInDate) / (1000 * 60 * 60 * 24));
      const nights = diffNights > 0 ? diffNights : (oldDetails.nights || 1);
      booking.bookingDetails = { ...oldDetails, checkIn, checkOut, nights };
      booking.changed('bookingDetails', true);
