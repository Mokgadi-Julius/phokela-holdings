...
router.get('/dashboard', auth, async (req, res) => {
  try {
    ...
    _weekStart.setDate(today.getDate() - today.getDay());
    const startOfWeek = _weekStart;
    ...
    const todayStr       = startOfToday.toISOString().substring(0, 10);
    const sevenDaysStr   = sevenDaysFromNow.toISOString().substring(0, 10);
    const upcomingBookings = await Booking.findAll({
        where: {
            [Op.and]: [
                sequelize.literal(`(
                    JSON_UNQUOTE(JSON_EXTRACT(\`bookingDetails\`, '$.checkIn'))   BETWEEN '${todayStr}' AND '${sevenDaysStr}'
                    OR JSON_UNQUOTE(JSON_EXTRACT(\`bookingDetails\`, '$.eventDate')) BETWEEN '${todayStr}' AND '${sevenDaysStr}'
                )`),
                { status: { [Op.in]: ['confirmed', 'pending'] } }
            ]
        },
        include: [{ model: Service, as: 'service', attributes: ['name', 'category'] }],
        order: [[sequelize.literal(`LEAST(
            COALESCE(JSON_UNQUOTE(JSON_EXTRACT(\`bookingDetails\`, '$.checkIn')),   '9999-12-31'),
            COALESCE(JSON_UNQUOTE(JSON_EXTRACT(\`bookingDetails\`, '$.eventDate')), '9999-12-31')
        )`), 'ASC']],
        limit: 10
    });
    ...
    const [dailyBookingsRaw, statusCountsRaw, activeRoomBookingsRaw] = await Promise.all([
      Booking.findAll({
      ...
      Booking.findAll({
        ...
        attributes: [
          'roomId',
          [sequelize.fn('SUM', sequelize.col('roomQuantity')), 'totalBooked'],
        ],
        group: ['roomId'],
        raw: true,
      }),
    ...
    // Build daily bookings array (fill zeros for days with no bookings)
    const dailyBookingsMap = {};
    dailyBookingsRaw.forEach(r => { dailyBookingsMap[r.date] = parseInt(r.count); });
    const dailyBookings = [];
    for (let i = 6; i >= 0; i--) {
      const d = new Date();
      ...
      dailyBookings.push({
        ...
        date:     dateStr,
        bookings: dailyBookingsMap[dateStr] || 0,
      });
    }

    // Booking counts by status
    const bookingStatusCounts = {};
    statusCountsRaw.forEach(r => { bookingStatusCounts[r.status] = parseInt(r.count); });

    // Active room bookings per room (pending + confirmed)
    const roomBookingCounts = {};
    activeRoomBookingsRaw.forEach(r => { roomBookingCounts[r.roomId] = parseInt(r.totalBooked) || 0; });
...
// @access  Private
router.get('/reports/bookings', auth, async (req, res) => {
  try {
