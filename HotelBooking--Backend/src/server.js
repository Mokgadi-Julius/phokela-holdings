...
const { runBlogSeed } = require('./scripts/seed-blog');
...
const corsOptions = {
  origin: function (origin, callback) {
    ...
    // In production, we can be specific, but for now, we echo the origin to ensure CORS works
...
// Re-enable helmet with relaxed settings for Railway
app.use(helmet({
...
const limiter = rateLimit({
  ...
  max: 2000, // Increased limit for both dev and prod to support dashboard polling
  message: {
    success: false,
    message: 'Too many requests from this IP, please try again later.'
  },
  validate: { trustProxy: true } // Enable trust proxy validation for Railway
});
app.use('/api/', limiter);
...
const server = app.listen(PORT, async () => {
  ...
  try {
    // Connect to MySQL Database and start schedulers after server starts
    await connectDB();
    await runBlogSeed();
    startRecurringExpensesScheduler();
    console.log('✅ Schedulers started');
  } catch (err) {
    console.error('❌ Failed to initialize database or schedulers:', err.message);
  }
